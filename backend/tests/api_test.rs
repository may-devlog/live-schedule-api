...
async fn create_test_app(pool: Pool<Sqlite>) -> axum::Router {
    ...
    axum::Router::new()
        ...
        .route("/schedules", axum::routing::get(require_auth_stub))
        .layer(cors)
        .layer(axum::extract::Extension(pool))
}

// 認証必須エンドポイントの簡易スタブ（Authorizationヘッダーが無ければ401を返す）
async fn require_auth_stub(headers: axum::http::HeaderMap) -> StatusCode {
