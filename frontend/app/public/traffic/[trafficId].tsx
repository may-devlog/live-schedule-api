// app/public/traffic/[trafficId].tsx - 公開ページ用の交通詳細画面（閲覧専用）
  ...
  useEffect(() => {
    ...
    const fetchTraffic = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch(getApiUrl(`/public/traffic/${trafficId}`));
        if (!res.ok) {
          if (res.status === 404) {
        ...
        if (data.schedule_id) {
          try {
            ...
            console.error("[PublicTrafficDetail] Failed to fetch schedule:", e);
          }
        }
