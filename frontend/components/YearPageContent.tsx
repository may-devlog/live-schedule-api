// components/YearPageContent.tsx - 年別ページの共通コンポーネント
...
type YearPageContentProps = {
  ...
  fetchStays?: (year: string) => Promise<any[]>;
...
export function YearPageContent({
  ...
  const [currentYear, setCurrentYear] = useState<string | null>(initialYear ?? null);
  ...
  useEffect(() => {
    ...
    const loadYear = async () => {
      try {
        ...
        // スケジュールを取得
        const data = await fetchSchedules(currentYear);
        
        // 各スケジュールの交通情報を並列で取得（往復フラグを考慮した金額計算用）
        const trafficMap = await fetchTrafficBySchedule(data, !isShared, shareId);
        ...
        // 宿泊情報を取得（showStayTabがtrueの場合のみ）
        if (showStayTab && fetchStays) {
          const allStays = await fetchStays(currentYear);
          if (isMounted) {
            setStays(allStays);
            // 予約サイトの色情報を即座に取得（エリアと同様に）
            if (allStays.length > 0) {
              try {
                const colorMap = await fetchWebsiteColors(allStays);
                if (isMounted) {
  ...
  useEffect(() => {
    ...
    const loadOptionsOrder = async () => {
      try {
        const { orderMap } = await loadSelectOptionsMap(shareId);
        if (isMounted) {
  ...
  const groupedStays = useMemo(() => {
    if (archiveType !== "宿泊" || !showStayTab) return [];
    return groupStays(stays, stayGroupingField, selectOptionsMap);
  }, [stays, stayGroupingField, selectOptionsMap, archiveType, showStayTab]);

  // グルーピング結果のタイトルの色を非同期で取得
  useEffect(() => {
    if (archiveType !== "イベント") {
    ...
    const loadStayGroupTitleColors = async () => {
      ...
      for (const stayGroup of groupedStays) {
        if (stayGroup.title && stayGroupingField !== "none") {
          ...
          if (optionType) {
            const cachedColor = getOptionColorSync(stayGroup.title, optionType);
            if (cachedColor) {
              initialColorMap.set(stayGroup.title, cachedColor);
            }
            
            colorPromises.push(
              getOptionColor(stayGroup.title, optionType).then((color) => {
                if (isMounted) {
                  initialColorMap.set(stayGroup.title, color);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* アーカイブタイプ選択（イベント / 宿泊） - showStayTabがtrueの場合のみ表示 */}
      {showStayTab && (
